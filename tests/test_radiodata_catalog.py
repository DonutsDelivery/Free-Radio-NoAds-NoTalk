...
class CatalogValidationTests(unittest.TestCase):
    ...
    def test_schema_is_generated_from_shared_constraints(self):
        ...
        self.assertEqual(duplicate_host["pattern"], r"^https?://[^/]+$")
...
class GenerationTests(unittest.TestCase):
    ...
    def test_all_exports_round_trip_with_exact_order_values_and_favorite_identities(self):
        ...
        expected_identities = [
            (station["host"], station["path"])
            for variable in radiodata_catalog.CATEGORY_VARIABLES
            for category in self.catalog[variable]
            for station in category["stations"]
        ]
        generated_identities = [
            (station["host"], station["path"])
            for variable in radiodata_catalog.CATEGORY_VARIABLES
            for category in generated_export(self.generated, variable)
            for station in category["stations"]
        ]
        self.assertEqual(generated_identities, expected_identities)
