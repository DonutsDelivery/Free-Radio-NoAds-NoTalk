...
Item {
    ...
    function setEnhancedMode(enabled) {
        console.log("SpectrumVisualizer: Enhanced mode", enabled ? "enabled" : "disabled")
        isPlaying = enabled
        
        if (enabled) {
            // Start enhanced spectrum simulation
            if (spectrumTimer) spectrumTimer.start()
        } else {
            // Stop and fade out
            if (spectrumTimer) spectrumTimer.stop()
            for (var i = 0; i < spectrumBars.length; i++) {
                spectrumBars[i].current = 0
                spectrumBars[i].peak = 0
                spectrumBars[i].peakHold = 0
            }
            spectrumCanvas.requestPaint()
        }
    }
    ...
    Text {
        ...
        anchors.margins: 4
        text: "Mode: " + (isPlaying ? "Enhanced Visualization" : "No Audio Source") + " | Playing: " + isPlaying + " | Vol: " + currentVolume.toFixed(2) + " | Bars: " + barCount
