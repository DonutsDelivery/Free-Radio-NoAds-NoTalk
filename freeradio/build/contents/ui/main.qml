...
PlasmoidItem {
    ...
    // Real audio engine with built-in spectrum analysis
    AudioEngine {
        id: player
        
        onSpectrumUpdated: {
            if (spectrumVisualizer) {
                spectrumVisualizer.updateFromEngine()
                console.log("AudioEngine: Spectrum updated")
            }
        }
        
        onPlaybackStateChanged: {
            console.log("AudioEngine: Playback state changed:", isPlaying())
        }
        
        // Properties to match MediaPlayer API
        property int playbackState: isPlaying() ? MediaPlayer.PlayingState : MediaPlayer.StoppedState
        property int mediaStatus: MediaPlayer.LoadedMedia
        property real volume: compactVolumeSlider.value
        ...
        // Methods to match MediaPlayer API
        function pause() { stop() }
        function play() { 
            if (source && source !== "") {
        ...
        onPlaybackStateChanged: {
            ...
            if (playbackState === MediaPlayer.StoppedState && 
                ...
                console.log("AudioStreamer: Restart attempts so far:", restartAttempts)
                ...
                if (restartAttempts < maxRestartAttempts) {
                    ...
                    Qt.callLater(function() {
                        if (currentStationUrl !== "" && !userPaused) {
                            console.log("AudioStreamer: Reinitializing connection after unexpected stop (attempt", restartAttempts, "of", maxRestartAttempts + ")")
                            player.stop()
                            player.source = ""
                            
                            // Use longer delay for retries to allow network recovery
                            Qt.callLater(function() {
                                player.source = currentStationUrl
                                fetchStreamMetadata(currentStationUrl)
                                lastBufferProgress = 0
                    ...
                    console.log("AudioStreamer: Maximum restart attempts reached. Stream may be unavailable.")
            ...
            // Reset restart counter on successful playback and start monitoring
    ...
    ColumnLayout {
        ...
        Rectangle {
            ...
            // Spectrum Visualizer Component (New - Working Version)
            SpectrumVisualizerNew {
